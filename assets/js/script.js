// Detect window size, if less than 1280px add class 'mobile' to posts-list therefore it will be auto hide when trigger the pjax request in small screen devices.
if ($(window).width() <= 1280) {
  $('#posts-list').addClass('mobile')
}
...
// If posts has class 'mobile', hide it after clicking.
$('.pl__all').on('click', function(){
  $(this).addClass('active').siblings().removeClass('active');
  if (posts.hasClass('mobile')) {
    $('#posts-list, #pjax, #icon-arrow').addClass('fullscreen');
  }
});
...
$('#avatar').on('click',function(){
  $('#pl__all').addClass('active').siblings().removeClass('active');
  $('.pl__all').delay(0).fadeIn(200);
  posts.addClass('fullscreen');
  button.addClass('fullscreen');
  content.delay(300).queue(function(){
    $(this).addClass('fullscreen').dequeue();});
...
// Pjax
$(document).pjax('#indexlist a, #avatar, #mobile-avatar, .pl__all', '#pjax', { fragment: '#pjax', timeout: 10000 });
$(document).on({
  'pjax:click': function() {
